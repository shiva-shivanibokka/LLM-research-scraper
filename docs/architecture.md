...
pgvector columns are fixed-dimension, so all vectors must come from one embedding model to be comparable. Embeddings are standardized on **OpenAI `text-embedding-3-small` (1536-dim)**, also BYOK. Consequence: the library/RAG features need an OpenAI key even if the LLM is Groq/Anthropic. Upgrade path: to change models, change three constants in `lib/models.ts`, migrate the `vector(N)` column, and re-embed.
